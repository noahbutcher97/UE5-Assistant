...
try {
    ...
    try {
        ...
        Write-Host "   WebClient failed, trying Invoke-WebRequest..." -ForegroundColor Yellow
    ...
    if ($FileSize -lt 10000) {
        Write-Host ""
        Write-Host "ERROR: Download Error: File too small ($FileSize bytes)" -ForegroundColor Red
    ...
    Write-Host "SUCCESS: Downloaded: $FileSizeMB MB in $($DownloadDuration.TotalSeconds.ToString('0.0'))s" -ForegroundColor Green
    ...
    try {
        ...
        Write-Host "   SUCCESS: Valid ZIP header detected" -ForegroundColor Green
        ...
        Write-Host "   SUCCESS: ZIP contains $entryCount entries" -ForegroundColor Green
        ...
        Write-Host "ERROR: ZIP validation failed: $_" -ForegroundColor Red
    ...
    Write-Host "SUCCESS: Extracted $FileCount files successfully" -ForegroundColor Green
    ...
    if (Test-Path $InitUnrealFile) {
        Write-Host "      WARNING: Overwriting existing init_unreal.py" -ForegroundColor Yellow
    }
    
    # Create init_unreal.py with correct content - NO EMOJIS to avoid encoding issues
    $InitUnrealScript = @'
"""
...
'@
    ...
    if (Test-Path $InitUnrealFile) {
        ...
        Write-Host "      SUCCESS: init_unreal.py created successfully ($InitFileSize bytes)" -ForegroundColor Green
    } else {
...
"@
    ...
    if (Test-Path $ManualStartFile) {
        ...
        Write-Host "      SUCCESS: _manual_start.py created successfully ($ManualFileSize bytes)" -ForegroundColor Green
    } else {
    ...
    if ($AllChecksPassed) {
        ...
        Write-Host ""
        Write-Host "   WARNING: Some files are missing - installation may be incomplete!" -ForegroundColor Yellow
    }
    ...
    if (Test-Path $TempZip) {
        ...
        if ($zipSize -lt 100000) {
            Write-Host ""
            Write-Host "WARNING: File is suspiciously small - likely an error page" -ForegroundColor Yellow
