...
def get_system_message(app_config: Dict[str, Any]) -> str:
    ...
    style_config = RESPONSE_STYLES.get(current_style,
...
def register_routes(app, app_config: Dict[str, Any], save_config_func):
    ...
    async def deploy_client(request: dict):
        ...
        try:
            ...
            return {
                ...
                True,
                "message":
                f"Deployed {len(copied_files)} files",
                "files_copied":
                copied_files,
                "overwrote_existing":
                overwrote_count if overwrote_count > 0 else None,
                "target_path":
                str(target_base),
                "instructions": [
        ...
        if client_dir.exists():
            for file_path in sorted(client_dir.rglob("*")):
                if file_path.is_file():
                    ...
                    files_data.append({
                        "path":
                        str(file_path.relative_to("ue5_client")),
                        "size":
                        len(content)
                    })
    ...
    async def execute_command(request: dict):
        ...
        try:
            ...
            if any(k in lower for k in [
                    ...
                    "what am i working on", "current project", "this project",
                    "project info", "breakdown"
            ]):
                # Signal UE5 to collect context and send for AI processing
                return {
                    "success": True,
                    "response":
                    f"[UE_CONTEXT_REQUEST] project_info|{user_input}"
                }

            # Blueprint capture requests - need AI interpretation
            if any(k in lower for k in [
                    "capture", "screenshot", "screen shot", "show blueprint",
                    "picture of", "image of"
            ]) and any(b in lower
                       for b in ["blueprint", "bp_", "graph", "node"]):
                return {
                    "success":
                    True,
                    "response":
                    f"[UE_CONTEXT_REQUEST] blueprint_capture|{user_input}"
                }

            # File/Asset browsing
            if any(k in lower for k in [
                    "show files", "list files", "browse files", "what files",
                    "project files"
            ]):
                return {
                    "success": True,
            ...
            print(f"[ERROR] {e}")
            error_msg = str(e)
            conversation.add_to_history(user_input, f"ERROR: {error_msg}",
                                        "execute_command", {"error": True})
    ...
    async def answer_with_context(request: dict):
        ...
        try:
            ...
            conversation.add_to_history(user_question, reply,
                                        "answer_with_context",
    ...
    async def wrap_natural_language(request: dict):
        ...
        try:
            ...
            conversation.add_to_history(summary_text, wrapped,
    ...
    async def describe_viewport(request: Request):
        ...
        print(
            f"[Describe Viewport] Style: {current_style}, Filter: {filter_mode}"
        )
        ...
        summary = generate_viewport_description(filtered_data, current_style,
                                                style_config, model,
                                                temperature)
        ...
        if not summary or len(summary.split()) < 4:
            ...
            cam_loc = camera_data.get("location", context.camera_location
                                      or [0, 0, 0])
            cam_rot = camera_data.get("rotation", context.camera_rotation
                                      or [0, 0, 0])
        ...
        conversation.add_to_history(user_prompt, summary, "describe_viewport",
                                    metadata)
    ...
    async def log_conversation(entry: ConversationEntry):
        ...
        conversation.add_to_history(entry.user_input, entry.assistant_response,
                                    entry.command_type, entry.metadata)
        return {
            "status": "logged",
            "total_entries": len(conversation.conversation_history)
        }
    ...
    async def search_files(request: dict):
        ...
        try:
            result = file_service.search_files(root_path, pattern, extension,
                                               max_results)
            return {"success": True, "data": result.model_dump()}
        except PermissionError as e:
            return {"success": False, "error": str(e), "type": "permission"}
    ...
    async def get_guidance(request: GuidanceRequest):
        ...
        try:
            ...
            conversation.add_to_history(
                request.query, guidance, "guidance", {
    ...
    async def list_blueprint_captures():
        ...
        return {
            ...
            "captures": [{
                ...
                capture.capture_id,
                "blueprint_name":
                capture.blueprint_name,
                "blueprint_path":
                capture.blueprint_path,
                "timestamp":
                capture.timestamp,
                "has_image":
                bool(capture.image_base64 or capture.image_url)
    ...
    async def register_project(request: dict):
        ...
        if "project_data" in request:
            ...
            project_data = {
                "name": name
                or path.split('/')[-2],  # Use folder name if no name
                "path": path,
                "version": version,
                "metadata": metadata
            }
...
When users ask about their project's actual data (file counts, blueprints, etc), ALWAYS use the appropriate [UE_REQUEST] to get real data instead of guessing. Respond ONLY with the [UE_REQUEST] token, nothing else."""
            ...
            response = openai.chat.completions.create(
                ...
                temperature=
                0.3,  # Lower temp for more consistent token detection
                max_tokens=1500)
                    ...
                    if project_id:
                        try:
                            ...
                            if ue5_response and ue5_response.get("success"):
                                ...
                                formatted_response = (
                                    format_response.choices[0].message.content
                                    or "").strip()
...
Return ONLY the JSON array, no explanation."""
    ...
    async def ue5_response(request: dict):
        ...
        if request_id:
            ...
            print(
                f"[HTTP Polling] Received response for request: {request_id}")
        ...
        try:
            ...
            for project_id, websocket in manager.ue5_clients.items():
                try:
                    ...
                    print(
                        f"✅ Emergency update sent to WebSocket: {project_id}")
                    total_notified += 1
                except Exception as e:
                    print(
                        f"❌ Failed to send emergency update to {project_id}: {e}"
                    )
            ...
            await manager.broadcast_to_dashboards({
                "type":
                "emergency_update_triggered",
                "message":
                f"Emergency update triggered for {total_notified} client(s)",
                "mode":
                "no_restart",
                "timestamp":
                datetime.now().isoformat()
