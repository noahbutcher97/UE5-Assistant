...
def main():
    parser = argparse.ArgumentParser(description='UE5 AI Assistant Automation Suite')
    subparsers = parser.add_subparsers(dest='command', help='Command to run')
    
    cleanup_parser = subparsers.add_parser('cleanup', help='Run cleanup operations')
    cleanup_parser.add_argument('--analyze', action='store_true', help='Analyze project for cleanup')
    cleanup_parser.add_argument('--auto', action='store_true', help='Run automatic cleanup')
    cleanup_parser.add_argument('--dry-run', action='store_true', default=True, help='Dry run mode (default: True)')
    cleanup_parser.add_argument('--force', action='store_true', help='Force cleanup (disables dry-run)')
    cleanup_parser.add_argument('--attached-assets', action='store_true', help='Clean attached_assets directory')
    ...
    ops_parser.add_argument('--verbose', action='store_true', help='Verbose output')
    
    list_parser = subparsers.add_parser('list', help='List available operations')
...
def run_cleanup(args, logger):
    ...
    if args.analyze:
        ...
        print("Please specify an action: --analyze, --auto, or --attached-assets")
