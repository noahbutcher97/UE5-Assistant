...
def _wrap_natural_language(factual_text: str) -> str:
    ...
    try:
        ...
        _log(f"🪶 Wrapping factual text ({len(factual_text)} chars)")
        result = _post_json(WRAP_ENDPOINT, payload)
        wrapped = result.get("response", factual_text)
        _log("✅ Natural-language wrapping successful.")
        return wrapped
    except Exception as e:
...
def send_and_store(user_text: str):
    ...
    try:
        ...
        _log(f"✅ Reply written to {LAST_REPLY_PATH}")
        return assistant_text

    except Exception as e:
        _log(f"❌ send_and_store failed: {e}")
        _log(traceback.format_exc())
